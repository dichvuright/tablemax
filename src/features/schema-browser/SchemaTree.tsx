...
export function SchemaTree() {
  ...
  const handleTableClick = (tableName: string) => {
    ...
    setSelectedCollection(tableName);
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="px-2 space-y-px">
        ...
        {!isMongo && tables.map((table) => (
          <div key={table.name}>
            <button
              className={`w-full flex items-center gap-1.5 px-2 py-1.5 rounded-md text-left transition-colors ${
                selectedCollection === table.name
                  ? 'bg-primary/15 text-primary'
                  : 'hover:bg-muted/30'
              }`}
              onClick={() => handleTableClick(table.name)}
              title={`Click to query: SELECT * FROM ${table.name}`}
            >
              <Table2 className={`h-3.5 w-3.5 shrink-0 ${
                selectedCollection === table.name ? 'text-primary/60' : 'text-muted-foreground/40'
              }`} />
              <span className="text-[14px] truncate">{table.name}</span>
            </button>
          </div>
        ))}
      </div>
